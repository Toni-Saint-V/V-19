...
import {
  Search, ShieldCheck, DownloadCloud, Settings,
  Users, Menu, X, SlidersHorizontal, LogOut, ArrowLeftRight
} from 'lucide-react';
...
type AdminNavSection = 'review' | 'export' | 'settings' | 'users';
  ...
  const renderNavContent = () => (
    <>
      <div className="flex items-center gap-2.5 px-2 pb-4 mb-2 border-b border-[#242529]">
        <div className="w-8 h-8 rounded-lg bg-orange-500 text-white flex items-center justify-center font-bold text-sm shadow-[0_0_15px_rgba(249,115,22,0.3)]">A</div>
      ...
      <div className="flex-1 overflow-y-auto px-2 py-4 space-y-5 scrollbar-hide">
        <nav className="space-y-0.5">
          ...
          <button onClick={() => navigateTo('review')} className={`w-full flex items-center gap-2.5 px-2.5 py-2 rounded-lg text-sm transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-orange-500 ${activeNav === 'review' ? 'bg-[#27272b] text-white border border-[#2e2f34]' : 'hover:bg-white/5 text-white/70 hover:text-white border border-transparent'}`}>
            <ShieldCheck className="w-4 h-4 text-orange-400/80" /> <span className="flex-1 text-left">Проверка</span>
            <span className="px-1.5 py-0.5 rounded-md bg-orange-500/20 text-orange-400 text-[11px] font-medium">2</span>
          </button>
          <button onClick={() => navigateTo('export')} className={`w-full flex items-center gap-2.5 px-2.5 py-2 rounded-lg text-sm transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-orange-500 ${activeNav === 'export' ? 'bg-[#27272b] text-white border border-[#2e2f34]' : 'hover:bg-white/5 text-white/70 hover:text-white border border-transparent'}`}>
            <DownloadCloud className="w-4 h-4 text-emerald-400/80" /> <span className="flex-1 text-left">Выгрузка</span>
            <span className="px-1.5 py-0.5 rounded-md bg-emerald-500/20 text-emerald-400 text-[11px] font-medium">3</span>
          </button>
        </nav>
