...
function StatusPill({ tone, children }: { tone: 'green' | 'orange' | 'blue' | 'neutral'; children: React.ReactNode }) {
  const toneClass = {
    ...
    blue: 'bg-[#3a45b4]/15 text-[#8fa3ff] border-[#3a45b4]/25',
...
function ManifestRow({ icon: Icon, label, value, state = 'ok' }: { icon: React.ElementType; label: string; value: string; state?: 'ok' | 'warn' | 'neutral' }) {
  const stateClass = state === 'ok' ? 'text-emerald-400' : state === 'warn' ? 'text-orange-400' : 'text-white/45';
  return (
...
export function AdminExportScreen() {
  ...
  return (
    <motion.div
      ...
      className="vf-admin-export-screen grid h-full min-h-[760px] grid-cols-1 gap-5 xl:grid-cols-[minmax(0,1fr)_390px]"
    >
      <section className="flex min-w-0 flex-col gap-5">
        <div className="vf-admin-export-metrics grid shrink-0 grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-4">
          <div className="rounded-2xl border border-[#242529] bg-gradient-to-br from-[#1a1a1d] to-[#141416] p-4 shadow-sm">
            <div className="flex items-center justify-between">
              <span className="text-[11px] font-medium uppercase tracking-wide text-emerald-400/80">Готовы</span>
          ...
          <div className="rounded-2xl border border-[#242529] bg-gradient-to-br from-[#1a1a1d] to-[#141416] p-4 shadow-sm">
            <div className="flex items-center justify-between">
              ...
              <PackageCheck className="h-4 w-4 text-[#8fa3ff]" />
            </div>
          ...
          <div className="rounded-2xl border border-[#242529] bg-gradient-to-br from-[#1a1a1d] to-[#141416] p-4 shadow-sm">
            <div className="flex items-center justify-between">
              <span className="text-[11px] font-medium uppercase tracking-wide text-orange-400/80">Pre-flight</span>
              {hasExportBlockers ? <XCircle className="h-4 w-4 text-red-400" /> : <ShieldCheck className="h-4 w-4 text-emerald-400" />}
            </div>
        ...
        <div className="vf-admin-export-list flex min-h-0 flex-1 flex-col overflow-hidden rounded-2xl border border-[#242529] bg-[#161617] shadow-[0_4px_24px_rgba(0,0,0,0.15)]">
              ...
              <div className="flex flex-1 items-center gap-2 lg:ml-auto lg:max-w-[520px]">
                <div className="relative flex-1">
                  ...
                  <input
                    ...
                    className="h-10 w-full rounded-xl border border-[#242529] bg-[#111113] pl-9 pr-3 text-[13px] text-white placeholder:text-white/30 outline-none focus:border-[#3a45b4]/70"
                  />
                </div>
          ...
          <div className="grid shrink-0 grid-cols-[44px_minmax(220px,1fr)_150px_130px_110px] gap-3 border-b border-[#242529] bg-[#141416] px-4 py-3 text-[10px] font-medium uppercase tracking-wider text-white/35 max-lg:hidden">
            <button
              onClick={toggleAll}
              className={`flex h-5 w-5 items-center justify-center rounded-md border ${items.every((item) => item.selected) && items.length > 0 ? 'border-[#3a45b4] bg-[#3a45b4]' : 'border-[#242529] bg-[#161617]'}`}
            >
              ...
              <div className="space-y-1">
                {items.map((item) => (
                  <button
                    ...
                    className={`grid w-full grid-cols-1 gap-3 rounded-xl border px-3 py-3 text-left transition-colors lg:grid-cols-[32px_minmax(220px,1fr)_150px_130px_110px] lg:items-center ${item.selected ? 'border-[#3a45b4]/35 bg-[#3a45b4]/10' : activeId === item.id ? 'border-white/10 bg-white/[0.035]' : 'border-transparent bg-transparent hover:border-white/5 hover:bg-white/5'}`}
                  >
                    <div className={`hidden h-5 w-5 shrink-0 items-center justify-center rounded-md border lg:flex ${item.selected ? 'border-[#3a45b4] bg-[#3a45b4]' : 'border-[#242529] bg-[#161617]'}`}>
                    ...
                    <div className="flex items-center gap-2">
                      <div className="h-2 flex-1 overflow-hidden rounded-full bg-white/5 lg:w-20 lg:flex-none">
                        <div className="h-full rounded-full bg-emerald-400" style={{ width: `${item.readiness}%` }} />
                      </div>
        ...
        <div className="vf-admin-export-mobile-dock" aria-label="Действия выгрузки">
          <button type="button" onClick={() => setMobileSummaryOpen(true)}>
            Сводка
          </button>
          <button type="button" onClick={handleExport} disabled={selectedCount === 0 || isExporting || hasExportBlockers}>
            {isExporting ? 'Архив...' : 'Выгрузить'}
          </button>
        </div>
              ...
              >
                ...
                <div className="vf-admin-sheet-metrics">
                  <div className="rounded-2xl border border-[#242529] bg-[#141416] p-4">
                    <div className="text-[11px] font-medium uppercase text-emerald-400/80">Готовы</div>
                    <div className="mt-3 text-2xl font-semibold text-white">{items.length}</div>
                    <div className="text-[11px] text-white/40">пакетов в очереди</div>
                  </div>
                  <div className="rounded-2xl border border-[#242529] bg-[#141416] p-4">
                    <div className="text-[11px] font-medium uppercase text-white/50">Выбрано</div>
                    <div className="mt-3 text-2xl font-semibold text-white">{selectedCount}</div>
                    <div className="text-[11px] text-white/40">{selectedApplicants} заявителей</div>
                  </div>
                  <div className="rounded-2xl border border-[#242529] bg-[#141416] p-4">
                    <div className="text-[11px] font-medium uppercase text-white/50">Документы</div>
                    <div className="mt-3 text-2xl font-semibold text-white">{selectedFiles}</div>
                    <div className="text-[11px] text-white/40">PDF/JPG/XLSX</div>
                  </div>
                  <div className="rounded-2xl border border-[#242529] bg-[#141416] p-4">
                    <div className="text-[11px] font-medium uppercase text-orange-400/80">Pre-flight</div>
                    <div className="mt-3 text-2xl font-semibold text-white">{hasExportBlockers ? 'STOP' : 'OK'}</div>
                    <div className="text-[11px] text-white/40">{selectedWarnings} предупреждений</div>
                  </div>
                </div>
                <div className="vf-questionnaire-info-note">
                  Контроль manifest, warnings и состава выгрузки вынесен сюда, чтобы список пакетов был первым рабочим экраном.
                </div>
              </motion.aside>
            </>
          )}
      ...
      <aside className="vf-admin-export-side flex min-h-0 flex-col overflow-hidden rounded-2xl border border-[#242529] bg-[#161617] shadow-[0_4px_24px_rgba(0,0,0,0.18)]">
        <div className="border-b border-[#242529] bg-[#1a1a1d] p-5">
          <div className="flex items-start justify-between gap-3">
            <div>
              <div className="text-[11px] font-medium uppercase tracking-wider text-emerald-400">Export cockpit</div>
              ...
              {selectedItems.length === 0 ? (
                ...
                selectedItems.map((item) => (
                  <div key={item.id} className="flex items-center gap-3 rounded-xl border border-white/5 bg-white/[0.025] p-3">
                    <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-[#3a45b4]/15 text-[#8fa3ff]">
        ...
        <div className="shrink-0 border-t border-[#242529] bg-[#1a1a1d] p-4">
          <button
            ...
            className="flex h-12 w-full items-center justify-center gap-2 rounded-xl bg-emerald-500 text-[14px] font-semibold text-white shadow-[0_0_24px_rgba(16,185,129,0.18)] transition-colors hover:bg-emerald-600 disabled:cursor-not-allowed disabled:bg-white/10 disabled:text-white/35 disabled:shadow-none"
          >
