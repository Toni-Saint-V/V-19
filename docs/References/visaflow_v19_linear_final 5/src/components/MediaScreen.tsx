...
export function MediaScreen() {
  ...
  const getStatusIcon = (status: FileStatus) => {
    switch (status) {
      ...
      case 'processing': return <Clock className="w-3.5 h-3.5 text-blue-400" />;
        ...
        <div className="flex items-center gap-2">
          <div className="relative w-full sm:w-[240px]">
            ...
            <input
              ...
              className="w-full h-10 bg-[#161617] border border-[#242529] rounded-[10px] pl-9 pr-3 text-sm text-white placeholder-white/40 focus:border-[#3a45b4] focus:ring-1 focus:ring-[#3a45b4]/30 transition-all outline-none"
            />
          </div>
          ...
          <AnimatePresence>
            {filteredFiles.map((file) => (
              <motion.div
                ...
                key={file.id}
                className="group flex flex-col bg-[#161617] border border-[#242529] rounded-2xl overflow-hidden hover:border-[#3a45b4]/40 transition-all cursor-pointer shadow-[0_4px_20px_rgba(0,0,0,0.1)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#3a45b4]"
                tabIndex={0}
              >
                {/* Preview Area */}
                <div className="aspect-[4/3] bg-[#101011] relative border-b border-[#242529] flex flex-col items-center justify-center overflow-hidden">
                  {file.status === 'error' && <div className="absolute inset-0 bg-red-500/5 mix-blend-overlay" />}
                  ...
                  <div className={`absolute bottom-2 left-2 px-2 py-1 rounded-md backdrop-blur-md border border-white/10 text-[10px] font-medium flex items-center gap-1.5 shadow-sm
                    ${file.status === 'verified' ? 'bg-emerald-500/10 text-emerald-400' :
                      file.status === 'processing' ? 'bg-blue-500/10 text-blue-400' : 'bg-red-500/10 text-red-400'}`}>
                ...
                <div className="p-3 bg-gradient-to-b from-[#1a1a1d] to-[#161617] flex-1 flex flex-col">
                  <div className="text-[13px] font-medium text-white truncate group-hover:text-[#8fa3ff] transition-colors" title={file.name}>
                    {file.name}
                  </div>
