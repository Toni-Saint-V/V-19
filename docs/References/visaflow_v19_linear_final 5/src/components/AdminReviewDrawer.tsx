...
import {
  ...
  User, Users, History, ScanText, MessageSquarePlus,
  AlertCircle, FileWarning, Info, ChevronDown
} from 'lucide-react';
...
const FieldRow = ({
  ...
  <div className={`flex flex-col lg:flex-row lg:items-center justify-between p-4 bg-[#1a1a1d] border rounded-xl gap-4 transition-colors
    ${status === 'error' ? 'border-orange-500/30 bg-orange-500/5' : 'border-[#242529] hover:border-[#2e2f34]'}
  `}>
    <div className="flex-1 min-w-0">
      <div className="flex items-center gap-2 mb-1">
        <span className="text-[11.5px] font-medium text-white/40 uppercase tracking-wider">{label}</span>
        {status === 'error' && <span className="px-1.5 py-0.5 rounded text-[10px] bg-orange-500/20 text-orange-400 font-medium">Есть замечание</span>}
        {status === 'ok' && <span className="px-1.5 py-0.5 rounded text-[10px] bg-emerald-500/20 text-emerald-400 font-medium">Проверено</span>}
      </div>
    ...
    <div className="flex items-center gap-2 shrink-0">
      {hasDocument && (
        <button
          onClick={onVerify}
          className="h-8 px-3 rounded-lg bg-[#3a45b4]/10 border border-[#3a45b4]/20 text-[#8fa3ff] text-[12px] font-medium hover:bg-[#3a45b4]/20 transition-colors flex items-center gap-1.5 focus-visible:ring-2 focus-visible:ring-[#3a45b4] outline-none"
        >
      ...
      <button
        onClick={onRemark}
        className="w-8 h-8 rounded-lg bg-orange-500/10 text-orange-400 flex items-center justify-center hover:bg-orange-500/20 transition-colors border border-transparent hover:border-orange-500/20 focus-visible:ring-2 focus-visible:ring-orange-500 outline-none"
      ...
      <button
        onClick={onApprove}
        className={`w-8 h-8 rounded-lg flex items-center justify-center transition-colors border focus-visible:ring-2 focus-visible:ring-emerald-500 outline-none
          ${status === 'ok' ? 'bg-emerald-500/20 text-emerald-400 border-emerald-500/30' : 'bg-white/5 text-white/40 border-transparent hover:bg-emerald-500/10 hover:text-emerald-400 hover:border-emerald-500/20'}`}
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-4 bg-white/[0.02] border border-white/5 rounded-2xl">
        <div>
          ...
          <select
            ...
            className="h-10 w-full sm:w-[240px] bg-[#1e1e21] border border-[#242529] rounded-xl px-3 text-[13px] text-white outline-none focus:border-[#3a45b4] focus:ring-1 focus:ring-[#3a45b4]/30"
          >
        ...
        <>
          ...
          >
              ...
              <div className="flex items-start justify-between gap-4 mb-5">
                <div className="min-w-0">
                  ...
                  <h2 className="text-[20px] lg:text-[24px] font-semibold text-white leading-tight tracking-tight flex items-center gap-3">
                    Проверка пакета
                    <span className="px-2.5 py-1 rounded-full bg-orange-500/10 border border-orange-500/20 text-orange-400 text-[11px] font-medium uppercase tracking-wide">
                      На проверке
                    </span>
                  </h2>
                </div>
                ...
                <div className="flex items-center gap-1.5 w-max mb-[-1px]">
                  {tabs.map(tab => (
                    <button
                      ...
                      `}
                    >
                      <tab.icon className={`w-4 h-4 ${activeTab === tab.id ? (tab.isWarning ? 'text-orange-400' : 'text-[#8fa3ff]') : 'opacity-70'}`} />
                      <span>{tab.label}</span>
                      {tab.count && (
                      ...
                      {activeTab === tab.id && (
                        <motion.div
                          ...
                          className="absolute bottom-0 inset-x-0 h-0.5 bg-[#3a45b4]"
                          initial={false}
            ...
            <footer className="p-4 lg:px-8 lg:py-5 border-t border-white/10 bg-[#111113]/90 backdrop-blur-md shrink-0 flex justify-end gap-3 pb-[max(16px,env(safe-area-inset-bottom))]">
              ...
              <button className="h-11 px-6 bg-emerald-500 hover:bg-emerald-600 text-white font-medium text-[13px] rounded-xl shadow-[0_0_20px_rgba(16,185,129,0.2)] transition-colors flex items-center gap-2">
