...
import {
  Plus, CheckCircle2, AlertCircle, Clock, FileText, ScanLine,
  UploadCloud, ChevronRight, User, Users, FileWarning, Search
} from 'lucide-react';
...
const DocCell = ({ status }: { status: DocStatus }) => {
  switch (status) {
    case 'verified':
      return (
        <div className="w-8 h-8 rounded-lg flex items-center justify-center bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 mx-auto" title="Подтверждено">
          <CheckCircle2 className="w-[18px] h-[18px]" />
        </div>
      );
    case 'processing':
      return (
        <div className="w-8 h-8 rounded-lg flex items-center justify-center bg-blue-500/10 border border-blue-500/20 text-blue-400 mx-auto" title="OCR распознавание...">
          <ScanLine className="w-[16px] h-[16px] animate-pulse" />
        </div>
      );
    case 'error':
      return (
        <div className="w-8 h-8 rounded-lg flex items-center justify-center bg-red-500/10 border border-red-500/30 text-red-400 mx-auto shadow-[0_0_10px_rgba(239,68,68,0.15)]" title="Ошибка проверки">
  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 lg:gap-4">
        ...
        <div className="p-4 lg:p-5 rounded-2xl bg-gradient-to-br from-[#1a1a1d] to-[#141416] border border-red-500/30 shadow-[0_4px_20px_rgba(239,68,68,0.05)] flex flex-col justify-between h-[110px] relative overflow-hidden group">
          <div className="absolute top-0 right-0 w-32 h-32 bg-red-500/5 rounded-full blur-2xl -mr-10 -mt-10" />
          <div className="flex items-center justify-between relative z-10">
            <span className="text-[12px] font-medium text-red-400/80 uppercase tracking-wide">Ошибки проверки</span>
            <div className="w-8 h-8 rounded-full bg-red-500/10 flex items-center justify-center">
              <FileWarning className="w-4 h-4 text-red-400" />
            </div>
          </div>
          <div className="relative z-10">
            <div className="text-2xl font-semibold text-white group-hover:text-red-400 transition-colors">2</div>
                  ...
                  <div className="flex items-center bg-[#1a1a1d] hover:bg-[#1e1e21] transition-colors border-b border-[#202124]">
                      ...
                      <div className="flex items-center justify-between">
                        <div>
                          ...
                          <div className="text-[11px] text-white/40 mt-1 flex items-center gap-2">
                            ...
                            <span className="text-orange-400/80">{sub.deadline}</span>
                          </div>
                        </div>
                      </div>
                    </div>
                    <div className="flex-1 flex justify-center py-3 px-6">
                      <div className="h-[2px] w-full bg-white/5 rounded-full relative overflow-hidden">
                        <div className={`absolute inset-y-0 left-0 bg-[#3a45b4]/40 ${sub.type === 'family' ? 'w-[45%]' : 'w-[20%]'}`} />
                      </div>
                    </div>
