...
import {
  Search, Plus, Filter, View, ArrowRight, User, Users, X,
  Menu, Bell, FileText, CheckCircle2, Clock, AlertCircle,
  Image as ImageIcon, FileWarning, ShieldCheck, DownloadCloud, UploadCloud, Settings
} from 'lucide-react';
...
type NavSection = 'submissions' | 'drafts' | 'applicants' | 'media' | 'issues' | 'settings';
...
export function CommandCenter({ onSwitchWorkspace }: { onSwitchWorkspace?: () => void }) {
  ...
  const getStatusBadge = (status: SubmissionStatus) => {
    switch (status) {
      case 'in_progress': return <span className="vf-status-chip is-muted"><Clock className="w-3 h-3" /> В работе</span>;
      case 'returned': return <span className="vf-status-chip is-danger"><AlertCircle className="w-3 h-3" /> Ошибки</span>;
      case 'submitted_for_review': return <span className="vf-status-chip is-review"><Clock className="w-3 h-3" /> На проверке</span>;
      case 'corrections_received': return <span className="vf-status-chip is-warning"><Clock className="w-3 h-3" /> Исправления</span>;
      case 'ready_for_export': return <span className="vf-status-chip is-success"><CheckCircle2 className="w-3 h-3" /> Готово</span>;
      default: return <span className="vf-status-chip is-muted"><FileText className="w-3 h-3" /> Черновик</span>;
    }
  };

  const getStatusDot = (status: SubmissionStatus) => {
    switch (status) {
      case 'in_progress': return 'vf-status-dot is-muted';
      case 'returned': return 'vf-status-dot is-danger';
      case 'submitted_for_review': return 'vf-status-dot is-review';
      case 'corrections_received': return 'vf-status-dot is-warning';
      case 'ready_for_export': return 'vf-status-dot is-success';
      default: return 'vf-status-dot is-muted';
    }
  };
      ...
      <div className="flex-1 overflow-y-auto px-2 space-y-5 scrollbar-hide">
        <nav className="space-y-0.5">
          ...
          <button onClick={() => navigateTo('issues')} className={`w-full flex items-center gap-2.5 px-2.5 py-2 rounded-lg text-sm transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#3a45b4] ${activeNav === 'issues' ? 'bg-[#27272b] text-white' : 'hover:bg-white/5 text-white/70 hover:text-white'}`}>
            ...
            <span className="px-1.5 py-0.5 rounded-full bg-[#22232a] border border-[#34364a] text-[#a8b2ff] text-[10px] font-semibold">2</span>
          </button>
          <button onClick={() => navigateTo('settings')} className={`w-full flex items-center gap-2.5 px-2.5 py-2 rounded-lg text-sm transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#3a45b4] ${activeNav === 'settings' ? 'bg-[#27272b] text-white' : 'hover:bg-white/5 text-white/70 hover:text-white'}`}>
              ...
              <div className="space-y-4 lg:space-y-6">
                <div className="flex flex-col sm:flex-row sm:items-center gap-3 min-h-[48px] lg:min-h-12">
                  <div className="flex bg-[#161617] p-1 border border-[#202124] rounded-[11px] overflow-x-auto scrollbar-hide">
                    ...
                    <button className="px-3 py-1.5 rounded-lg text-sm text-white/50 hover:text-white/80 transition-colors whitespace-nowrap focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#3a45b4] border border-transparent">Ошибки <span className="ml-1.5 text-[10px] bg-red-500/20 text-red-400 px-1.5 py-0.5 rounded-md">3</span></button>
                  ...
                  <div className="sm:ml-auto flex items-center gap-2 w-full sm:w-auto">
                    <div className="relative w-full sm:w-[260px]">
                      ...
                      <input type="text" placeholder="Поиск..." className="w-full h-10 bg-[#1e1e21] border border-[#242529] rounded-[10px] pl-9 pr-3 text-sm text-white placeholder-white/40 focus:border-[#3a45b4] focus:ring-1 focus:ring-[#3a45b4]/30 transition-all outline-none" />
                    </div>
