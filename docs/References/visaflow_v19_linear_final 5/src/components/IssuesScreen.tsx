...
import { AlertCircle, FileWarning, ArrowRight, User, Users, CheckCircle2, ChevronRight, Check } from 'lucide-react';
  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-2 gap-3 lg:gap-4">
        <div className="p-4 lg:p-5 rounded-2xl bg-red-500/5 border border-red-500/20 shadow-[0_4px_24px_rgba(239,68,68,0.05)] flex flex-col justify-between h-[100px] relative overflow-hidden group">
          <div className="absolute top-0 right-0 w-24 h-24 bg-red-500/10 rounded-full blur-2xl -mr-10 -mt-10 transition-transform group-hover:scale-110" />
          <div className="flex items-center justify-between relative z-10">
            <span className="text-[12px] font-medium text-red-400 uppercase tracking-wide">Критичные ошибки</span>
            <AlertCircle className="w-5 h-5 text-red-400" />
          </div>
          <div className="relative z-10 text-2xl font-semibold text-white">{criticalCount}</div>
        </div>

        <div className="p-4 lg:p-5 rounded-2xl bg-orange-500/5 border border-orange-500/20 flex flex-col justify-between h-[100px] relative overflow-hidden group">
          <div className="absolute top-0 right-0 w-24 h-24 bg-orange-500/10 rounded-full blur-2xl -mr-10 -mt-10 transition-transform group-hover:scale-110" />
          <div className="flex items-center justify-between relative z-10">
            <span className="text-[12px] font-medium text-orange-400 uppercase tracking-wide">Предупреждения</span>
      ...
      <div className="space-y-3">
        {mockIssues.map((issue) => (
          <div
            ...
            className="group flex flex-col sm:flex-row gap-4 p-4 lg:p-5 bg-[#161617] border border-[#242529] hover:border-[#3a45b4]/40 rounded-2xl cursor-pointer transition-all shadow-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#3a45b4]"
            tabIndex={0}
          >
            {/* Severity Indicator */}
            <div className={`w-10 h-10 shrink-0 rounded-xl flex items-center justify-center border shadow-inner mt-1
              ${issue.type === 'critical' ? 'bg-red-500/10 border-red-500/20 text-red-400' : 'bg-orange-500/10 border-orange-500/20 text-orange-400'}`}>
            ...
            <div className="flex-1 min-w-0 flex flex-col justify-center">
              ...
              <h3 className="text-[15px] font-semibold text-white mb-1.5 group-hover:text-[#8fa3ff] transition-colors">{issue.title}</h3>
            ...
            <div className="flex items-center sm:justify-end mt-2 sm:mt-0 shrink-0">
              <button className="w-full sm:w-auto px-5 h-10 rounded-xl text-[13px] font-medium transition-colors flex items-center justify-center gap-2 bg-[#1a1a1d] border border-[#242529] text-white group-hover:border-[#3a45b4]/50 group-hover:bg-[#3a45b4]/10 group-hover:text-[#8fa3ff]">
