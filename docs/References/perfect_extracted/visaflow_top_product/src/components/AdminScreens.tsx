...
function ReviewQueueCard({ item, onOpenDrawer }: { item: ReviewCard; onOpenDrawer: (id: string) => void }) {
  ...
  return (
    <button
      ...
      className={`group w-full rounded-2xl border p-4 text-left transition-all hover:-translate-y-0.5 hover:shadow-[0_14px_40px_rgba(0,0,0,0.22)] ${hasBlocker ? 'border-[#5b2b32]/45 bg-[#1d1719]/80 hover:border-[#74414a]/55' : 'border-[#242529] bg-[#161617] hover:border-[#6f64ff]/40'}`}
    >
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="grid h-full min-h-[760px] grid-cols-1 gap-5 xl:grid-cols-[minmax(0,1fr)_360px]">
      <section className="min-w-0 space-y-5">
        <div className="rounded-3xl border border-[#242529] bg-gradient-to-br from-[#1a1a1d] via-[#161617] to-[#101011] p-5 lg:p-6">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            ...
            <button onClick={() => visibleReviews[0] && onOpenDrawer(visibleReviews[0].id)} className="flex h-11 items-center justify-center gap-2 rounded-xl bg-[#6f64ff] px-4 text-[13px] font-semibold text-white shadow-[0_0_24px_rgba(58,69,180,0.25)] transition-colors hover:bg-[#4855d4]">
        ...
        <div className="rounded-2xl border border-[#242529] bg-[#161617]">
          <div className="flex flex-col gap-3 border-b border-[#242529] p-4 lg:flex-row lg:items-center">
            <div className="flex flex-wrap items-center gap-2">
              <button onClick={() => setActiveLane('all')} className={`rounded-xl border px-3 py-2 text-[12px] font-medium transition-colors ${activeLane === 'all' ? 'border-[#6f64ff]/40 bg-[#6f64ff]/15 text-[#b8baff]' : 'border-white/10 bg-white/5 text-white/55 hover:text-white'}`}>Все</button>
              {lanes.map((lane) => {
            ...
            <div className="flex flex-1 items-center gap-2 lg:ml-auto lg:max-w-[440px]">
              <div className="relative flex-1">
                ...
                <input className="h-10 w-full rounded-xl border border-[#242529] bg-[#111113] pl-9 pr-3 text-[13px] text-white placeholder:text-white/30 outline-none focus:border-[#6f64ff]/55" placeholder="Поиск: ID, агент, семья" />
              </div>
      ...
      <aside className="flex min-h-0 flex-col gap-5">
        <div className="rounded-2xl border border-[#242529] bg-[#161617] p-5">
          ...
          <div className="space-y-3">
            <div className="rounded-xl border border-[#6f64ff]/25 bg-[#6f64ff]/10 p-3">
