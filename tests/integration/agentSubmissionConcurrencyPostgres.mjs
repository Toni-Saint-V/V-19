...
const adminId = "00000000-0000-4000-8000-0000000000b1";
const unapprovedUserId = "00000000-0000-4000-8000-0000000000a2";
const issueGuardIssueId = "зм-submission-issue-guard-domain-1";
const issueGuardCorrectionId = stableUuid(
  `correction:submission-issue-guard:${issueGuardIssueId}`,
);
const mediaGuardIssueId = "зм-submission-media-issue-guard-domain-1";
const mediaGuardCorrectionId = stableUuid(
  `correction:submission-media-issue-guard:${mediaGuardIssueId}`,
);
const mediaGuardAssetId =
  "media-submission-media-issue-guard-applicant-media-issue-guard-passport_scan";
const mediaGuardOldFileName = "old-passport.jpg";
const mediaGuardNewFileName = "new-passport.jpg";
const mediaGuardOldPath =
  "submissions/submission-media-issue-guard/applicants/applicant-media-issue-guard/passport_scan/old-passport.jpg";
const mediaGuardNewPath =
  "submissions/submission-media-issue-guard/applicants/applicant-media-issue-guard/passport_scan/new-passport.jpg";
...
create table public.corrections (
  ...
  applicant_id text references public.applicants(id),
  scope text not null default 'submission',
  field_key text,
  media_type text,
  reason text not null default 'Test review issue',
  severity text not null default 'blocking',
  status text not null default 'open',
  created_by uuid not null default '${adminId}'::uuid
...
insert into public.submissions (
  ...
  'submission-issue-guard',
  '${agentId}',
  'returned',
  'Issue ownership guard',
  '{
    "v19CockpitSnapshot":{
      "submission":{
        "issues":[{
          "comment":"Проверьте документ",
          "createdAt":"2026-07-29T09:00:00Z",
          "createdBy":"admin",
          "id":"${issueGuardIssueId}",
          "reason":"Исправьте номер",
          "severity":"blocker",
          "snapshot":"",
          "status":"open",
          "target":{
            "applicantId":"applicant-issue-guard",
            "applicantName":"TEST first-name TEST surname",
...
insert into public.corrections (
  ...
  '${issueGuardCorrectionId}',
  'submission-issue-guard',
  'applicant-issue-guard',
  'field',
  'stay-purpose-details',
  'Исправьте номер — Проверьте документ',
  'blocking',
  'open',
  '${adminId}',
...
insert into public.submissions (
  ...
  'submission-media-issue-guard',
  '${agentId}',
  'returned',
  'Media issue ownership guard',
  '{
    "v19CockpitSnapshot":{
      "submission":{
        "issues":[{
          ...
          "createdAt":"2026-07-29T09:00:00Z",
          "createdBy":"admin",
          "id":"${mediaGuardIssueId}",
          "reason":"Скан нечитаемый",
          "severity":"blocker",
          "snapshot":"media-evidence:v1:[\"submission-media\",\"${mediaGuardOldPath}\"]",
          "status":"open",
          "target":{
            "applicantId":"applicant-media-issue-guard",
            "applicantName":"TEST first-name TEST surname",
...
insert into public.corrections (
  ...
  '${mediaGuardCorrectionId}',
  'submission-media-issue-guard',
  'applicant-media-issue-guard',
  'media',
  'passport_scan',
  'Скан нечитаемый — Загрузите новый скан',
  'blocking',
  'open',
  '${adminId}',
  '2026-07-29T09:00:00Z'
...
values
  ('submission-media', '${mediaGuardOldPath}'),
  ('submission-media', '${mediaGuardNewPath}');
insert into public.applicants (id, submission_id)
values
...
begin
  ...
  loop
    insert into public.corrections (
      ...
      correction_record.field_key,
      correction_record.media_type,
      correction_record.reason,
      correction_record.severity,
      correction_record.status,
      correction_record.created_by,
      correction_record.created_at,
      correction_record.fixed_at
    )
    on conflict (id) do update
...
function questionnaireValue(fieldId) {
  ...
  if (["arrival-date", "desired-date-1"].includes(fieldId)) {
    return "10.08.2026";
  }
  if (["departure-date", "desired-date-2"].includes(fieldId)) {
    return "20.08.2026";
  }
  ...
  return applicants.flatMap((applicant) => {
    ...
    return types.map((type) => {
      ...
      return {
        ...
        storage_path: `submissions/${id}/applicants/${applicant.id}/${type}/${generatedFileName}`,
        submission_id: id,
        type,
...
function mutationPayload({
  ...
  const normalizedQuestionnaire =
    questionnaireAnswers ?? completeQuestionnaireAnswers(id, normalizedApplicants);
  ...
  const cockpitSubmission = {
    agentId,
    applicants: snapshotApplicants(normalizedApplicants, normalizedQuestionnaire),
...
function issueGuardIssue(overrides = {}) {
  const { target: targetOverrides, ...issueOverrides } = overrides;
...
function issueGuardCorrection(overrides = {}) {
  return {
    applicant_id: "applicant-issue-guard",
    created_at: "2026-07-29T09:00:00Z",
    created_by: agentId,
    field_key: "stay-purpose-details",
    fixed_at: null,
    id: issueGuardCorrectionId,
    media_type: null,
    reason: "Исправьте номер — Проверьте документ",
    scope: "field",
    severity: "blocking",
    status: "open",
    submission_id: "submission-issue-guard",
    ...overrides,
  };
}

function issueGuardMutationPayload({
  correctionOverrides = {},
  corrections,
  fixed = false,
  issueOverrides = {},
  issues,
  targetCorrected = fixed,
} = {}) {
  const applicants = normalizeApplicants("submission-issue-guard", [
    {
      id: "applicant-issue-guard",
      passport_number: "761234567",
      role: "Основной заявитель",
      submission_id: "submission-issue-guard",
    },
  ]);
  const questionnaireAnswers = [
    ...completeQuestionnaireAnswers("submission-issue-guard", applicants),
    {
      applicant_id: "applicant-issue-guard",
      field_id: "stay-purpose-details",
      label: "Уточнение цели",
      section_id: "applicant-issue-guard-trip",
      submission_id: "submission-issue-guard",
      updated_by: agentId,
      value: targetCorrected ? "Tourism details" : "",
    },
  ];
  const issue = issueGuardIssue({
    ...(fixed ? { status: "fixed_by_agent" } : {}),
    ...issueOverrides,
  });
  const correction = issueGuardCorrection({
    ...(fixed
      ? {
          fixed_at: "2026-07-29T10:00:00Z",
          status: "fixed",
        }
      : {}),
    ...correctionOverrides,
  ...
  return mutationPayload({
    ...
    id: "submission-issue-guard",
    issues: issues ?? [issue],
    questionnaireAnswers,
    status: "returned",
    title: "Issue ownership guard",
  });
}

function mediaGuardIssue(overrides = {}) {
  return {
    comment: "Загрузите новый скан",
    createdAt: "2026-07-29T09:00:00Z",
    createdBy: "admin",
    ...
    reason: "Скан нечитаемый",
    severity: "blocker",
    snapshot: `media-evidence:v1:${JSON.stringify([
      "submission-media",
      mediaGuardOldPath,
    ])}`,
    status: "open",
...
function mediaGuardCorrection(overrides = {}) {
  return {
    applicant_id: "applicant-media-issue-guard",
    created_at: "2026-07-29T09:00:00Z",
    created_by: agentId,
    field_key: null,
    fixed_at: null,
    id: mediaGuardCorrectionId,
    media_type: "passport_scan",
    reason: "Скан нечитаемый — Загрузите новый скан",
    scope: "media",
    severity: "blocking",
    status: "open",
    submission_id: "submission-media-issue-guard",
    ...overrides,
  };
}

function mediaGuardMutationPayload({
  fixed = true,
  generatedFileName = mediaGuardNewFileName,
  issueOverrides = {},
  storagePath = mediaGuardNewPath,
} = {}) {
  const applicants = normalizeApplicants("submission-media-issue-guard", [
    {
      id: "applicant-media-issue-guard",
      passport_number: "761234567",
      role: "Основной заявитель",
      submission_id: "submission-media-issue-guard",
    },
  ...
  return mutationPayload({
    ...
    id: "submission-media-issue-guard",
    issues: [
      mediaGuardIssue({
        ...(fixed ? { status: "fixed_by_agent" } : {}),
        ...issueOverrides,
    ...
    mediaAssets,
    questionnaireAnswers: completeQuestionnaireAnswers(
      "submission-media-issue-guard",
      applicants,
    ),
    status: "returned",
    title: "Media issue ownership guard",
  });
}
...
try {
  ...
  const sqlIssueIdProof = psql(`
set role authenticated;
set request.jwt.claim.sub = '${agentId}';
select app_private.agent_issue_correction_id(
  'submission-issue-guard',
  '${issueGuardIssueId}'
)::text;
`);
  if (sqlIssueIdProof !== issueGuardCorrectionId) {
    throw new Error(
      `SQL issue identity ${sqlIssueIdProof} diverges from client ${issueGuardCorrectionId}`,
    );
  }

  const forgedIssueId = "зм-submission-issue-guard-forged";
  const forgedIssue = issueGuardIssue({
    id: forgedIssueId,
    reason: "Forged Agent issue",
  });
  const forgedCorrection = issueGuardCorrection({
    id: stableUuid(`correction:submission-issue-guard:${forgedIssueId}`),
    reason: "Forged Agent issue — Проверьте документ",
  });
  const rejectedIssueMutations = [
    issueGuardMutationPayload({ corrections: [], issues: [] }),
    issueGuardMutationPayload({
      corrections: [issueGuardCorrection(), forgedCorrection],
      issues: [issueGuardIssue(), forgedIssue],
    }),
    issueGuardMutationPayload({
      correctionOverrides: { field_key: "passport-expiry-date" },
      issueOverrides: { target: { field: "passport-expiry-date" } },
    }),
    issueGuardMutationPayload({
      correctionOverrides: {
        reason: "Agent rewrote reason — Проверьте документ",
      },
      issueOverrides: { reason: "Agent rewrote reason" },
    }),
    issueGuardMutationPayload({
      correctionOverrides: { severity: "note" },
      issueOverrides: { severity: "warning" },
    }),
    issueGuardMutationPayload({
      fixed: true,
    ...
    issueGuardMutationPayload({
      ...
      issueOverrides: { status: "closed_by_admin" },
    }),
    issueGuardMutationPayload({
      correctionOverrides: {
        id: stableUuid("correction:submission-issue-guard:зм-spoofed-identity"),
      },
      issueOverrides: { id: "зм-spoofed-identity" },
    }),
  ];

  rejectedIssueMutations.forEach((payload, index) => {
    assertInsufficientPrivilege(
      callAgentMutation({
        expectedRevision: 3,
        operationId: `00000000-0000-4000-8000-000000000c0${index + 1}`,
        payload,
      }),
    );
  });

  const rejectedIssueMutationProof = psql(`
select
  field_key || ':' ||
  reason || ':' ||
  severity || ':' ||
  status || ':' ||
  created_by::text || ':' ||
  coalesce(fixed_at::text, 'not-fixed')
from public.corrections
where id = '${issueGuardCorrectionId}';
`);
  if (
    rejectedIssueMutationProof !==
    `stay-purpose-details:Исправьте номер — Проверьте документ:blocking:open:${adminId}:not-fixed`
  ) {
    throw new Error(
      `Rejected Agent issue mutation changed durable Admin data: ${rejectedIssueMutationProof}`,
    );
  }
...
select ${callAgentMutation({
    expectedRevision: 3,
    operationId: "00000000-0000-4000-8000-000000000c10",
    payload: issueGuardMutationPayload({ fixed: true }),
  })};
`);
  const fixedIssueProof = psql(`
select
  correction.field_key || ':' ||
  correction.reason || ':' ||
  correction.severity || ':' ||
  correction.status || ':' ||
  correction.created_by::text || ':' ||
  (correction.fixed_at is not null)::text || ':' ||
  (
    submission.family_intelligence
      #>> '{v19CockpitSnapshot,submission,issues,0,status}'
  )
from public.corrections as correction
join public.submissions as submission
  on submission.id = correction.submission_id
where correction.id = '${issueGuardCorrectionId}';
`);
  if (
    fixedIssueProof !==
    `stay-purpose-details:Исправьте номер — Проверьте документ:blocking:fixed:${adminId}:true:fixed_by_agent`
  ) {
    throw new Error(
      `Legitimate Agent issue fix did not preserve Admin ownership: ${fixedIssueProof}`,
    );
  }
  ...
  assertInsufficientPrivilege(
    callAgentMutation({
      expectedRevision: 7,
      operationId: "00000000-0000-4000-8000-000000000c11",
      payload: mediaGuardMutationPayload({
        generatedFileName: mediaGuardOldFileName,
        storagePath: mediaGuardOldPath,
      }),
    }),
  );
  const missingMediaFileName = "missing-passport.jpg";
  assertInsufficientPrivilege(
    callAgentMutation({
      expectedRevision: 7,
      operationId: "00000000-0000-4000-8000-000000000c12",
      payload: mediaGuardMutationPayload({
        generatedFileName: missingMediaFileName,
        storagePath: `submissions/submission-media-issue-guard/applicants/applicant-media-issue-guard/passport_scan/${missingMediaFileName}`,
      }),
    }),
  );
  const rejectedMediaMutationProof = psql(`
select
  media.storage_path || ':' ||
...
join public.corrections as correction
  on correction.submission_id = media.submission_id
where media.id = '${mediaGuardAssetId}'
  and correction.id = '${mediaGuardCorrectionId}';
`);
  if (rejectedMediaMutationProof !== `${mediaGuardOldPath}:replace_required:open`) {
    throw new Error(
      `Rejected media correction changed durable Admin data: ${rejectedMediaMutationProof}`,
    );
  }
...
select ${callAgentMutation({
    expectedRevision: 7,
    operationId: "00000000-0000-4000-8000-000000000c13",
    payload: mediaGuardMutationPayload(),
  })};
`);
  const fixedMediaProof = psql(`
select
  media.storage_path || ':' ||
  media.review_status::text || ':' ||
...
join public.corrections as correction
  on correction.submission_id = media.submission_id
join public.submissions as submission
  on submission.id = media.submission_id
where media.id = '${mediaGuardAssetId}'
  and correction.id = '${mediaGuardCorrectionId}';
`);
  if (
    fixedMediaProof !==
    `${mediaGuardNewPath}:not_reviewed:passport_scan:Скан нечитаемый — Загрузите новый скан:fixed:${adminId}:fixed_by_agent`
  ) {
    throw new Error(
      `Legitimate media correction did not preserve Admin ownership/evidence: ${fixedMediaProof}`,
    );
  }
...
`);
  if (
    freshDraftProof !==
    [`${freshFamilyId}:draft:2:не указано`, `${freshSingleId}:draft:1:не указано`].join(
      "\n",
    )
  ) {
  ...
  const partialFamilyQuestionnaire = completeQuestionnaireAnswers(
    ...
    if (["arrival-date", "departure-date", "stay-duration"].includes(answer.field_id)) {
      return { ...answer, value: "" };
    }
    return answer;
  });
...
`);
  if (sequentialFamilyProof !== "Family dates complete:2:10.08.2026 - 20.08.2026") {
    throw new Error(
...
`);
  ...
  const reviewFamilyQuestionnaire = completeQuestionnaireAnswers(
    ...
    .map((answer) =>
      answer.applicant_id === "family-spouse" && answer.field_id === "occupation"
        ? { ...answer, value: "MINOR" }
        : answer,
    )
    .filter(
  ...
  const reviewFamilyMedia = completeMediaAssets(validFamilyId, reviewFamilyApplicants);
...
`)
  ...
  if (concurrentProof !== "Concurrent winner:2") {
    throw new Error(
      `Concurrent Agent CAS did not preserve the winner: ${concurrentProof}`,
    );
  }
...
`);
  ...
  const fillingMedia = completeMediaAssets("submission-filling", fillingApplicants);
  ...
  const phantomMedia = completeMediaAssets("submission-phantom", phantomApplicants);
  ...
  const approvedAnswerApplicants = normalizeApplicants("submission-approved-answer", [
    {
  ...
  const correctedApprovedQuestionnaire = completeQuestionnaireAnswers(
    ...
    answer.field_id === "email" ? { ...answer, value: correctedEmail } : answer,
  );
...
`);
  ...
  const exportReadyApplicants = normalizeApplicants("submission-export-ready", [
    {
