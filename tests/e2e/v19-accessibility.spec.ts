      ...
      .poll(
        () =>
          dialog
            ...
            .evaluateAll((nodes) =>
              (
                nodes as unknown as Array<{ getAttribute(name: string): string | null }>
              ).every((node) =>
                /opacity:\s*1(?:;|$)/.test(node.getAttribute("style") ?? ""),
              ),
            ),
        {
    ...
    .poll(
      () =>
        page.locator('[data-testid="agent-action-row"]').evaluateAll((rows) =>
          (
            rows as unknown as Array<{ getAttribute(name: string): string | null }>
          ).every((row) => {
            const inlineStyle = row.getAttribute("style") ?? "";
            return (
              !inlineStyle.includes("opacity") ||
              /opacity:\s*1(?:;|$)/.test(inlineStyle)
            );
          }),
        ),
...
test.describe("V-19 accessibility contract", () => {
  ...
  test("admin primary surfaces, export, and submission drawer have no automated WCAG A/AA violations", async ({
    ...
    await expect(
      page.getByRole("heading", { name: /^(Очередь на проверку|Проверка)$/ }),
    ).toBeVisible();
