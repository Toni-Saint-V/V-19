...
export function productionDraftSnapshotMutationFromBaseline(
  ...
  if (intent.mode === "add_issue") {
    const target = snapshotFieldTarget(submission, intent.fieldLabel);
    ...
    const expectedContentDigest = persistedSnapshotContentDigest(next);
    if (
      next === submission ||
      !issue ||
      issue.status !== "open" ||
  ...
  if (
    ...
    target.field.error !== intent.reason
  ) {
    return null;
  }
  const result = markSubmissionIssueFixedResult(submission, issue.id, "agent");
  if (!result.ok) return null;
  const expectedContentDigest = persistedSnapshotContentDigest(result.data);
  if (!expectedContentDigest) return null;
  return {
    expectedContentDigest,
    fieldError: {
      applicantId: target.applicantId,
...
function questionnaireAnswersMatch(
  ...
  for (const expected of contract.draft.questionnaireAnswers) {
    ...
    if (
      !actual ||
      actual.labelDigest !== expected.labelDigest ||
