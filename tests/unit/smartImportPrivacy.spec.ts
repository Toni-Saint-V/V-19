...
describe("smart import privacy boundary", () => {
  ...
  test("ships readable local Russian OCR language data", () => {
    const asset = join(
      repositoryRoot,
      "public",
      "tesseract",
  ...
  test("documents that only confirmed fields persist", () => {
    const contract = readFileSync(
      join(repositoryRoot, "docs", "release", "canonical-domain-contract.md"),
      "utf8",
    );

    expect(contract).toContain("### 1.9 Universal Smart Import privacy boundary");
    expect(contract).toContain("Only agent-selected questionnaire values may persist");
    expect(contract).toContain("must never populate an address dictionary");
  });
});
