...
function surfaceVisibleCopy(
  surface: ReturnType<typeof buildSubmissionAiHelperSurface>,
) {
  return [
    surface.ariaLabel,
    surface.title,
...
describe("local Case Copilot", () => {
  ...
  test("keeps the full AI helper surface free from unsafe trust copy", () => {
    const surface = buildSubmissionAiHelperSurface({
      role: "agent",
      submission: byId("ПД-1048"),
      surface: "agent",
    ...
    expect(surfaceVisibleCopy(surface)).not.toMatch(forbiddenTrustCopy);
  });
});
