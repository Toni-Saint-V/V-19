...
describe("ReviewWorkspace passport section contract", () => {
  ...
  test("reports open and corrected passport issues as separate counts", () => {
    const source = singleSubmission();
    const applicant = source.applicants[0];
    if (!applicant) throw new Error("Expected review applicant.");
    const passportIssue = {
      comment: "Проверьте значение.",
      createdAt: "2026-07-26T12:00:00.000Z",
      createdBy: "admin",
      reason: "Есть расхождение",
    ...
    const submission: Submission = {
      ...source,
      issues: [
        { ...passportIssue, id: "open-passport-number", status: "open" },
        {
          ...passportIssue,
          id: "fixed-passport-number",
          status: "fixed_by_agent",
        },
      ],
      status: "corrections_received",
    ...
    expect(reviewStatus).toHaveTextContent("Открыто 1");
    expect(reviewStatus).toHaveTextContent("К закрытию 1");
    expect(reviewStatus).not.toHaveTextContent("Открыто 2");
  });
