...
describe("local demo media replacement", () => {
  test("rejects a replacement target that could overwrite the committed blob", async () => {
    const target = replacementTarget();
    const storeMedia = vi.fn(async () => ({ path: target.path }));
    ...
    ).rejects.toThrow("Новый файл совпал с текущим адресом хранения");
