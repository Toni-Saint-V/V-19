...
describe("local-demo bundled media integrity", () => {
  ...
  test("rejects stored media outside the canonical submission target before loading bytes", async () => {
    const source = readySeedSubmission();
    const passport = source.files.find(
      (file) => file.type === "passport_scan" && file.status === "accepted",
    );
    if (!passport) throw new Error("Missing seed passport");
    const generatedFileName = "guarded_passport_scan.jpg";
    const target = buildMediaStoragePath(
      source.id,
      passport.applicantId,
      "passport_scan",
      generatedFileName,
    );
    const stored = new Blob(["same-size-media"], { type: "image/jpeg" });
    const tamperedPassport: SubmissionFile = {
      ...passport,
      generatedFileName,
      ...
      storagePath: target.path.replace(source.id, "foreign-submission"),
    ...
    const loadMedia = vi.fn(async () => stored);
    const options = buildLocalDemoExportMediaZipOptions([submission], loadMedia);
    const asset = options.documentAssets?.find(
      (candidate) => candidate.type === "passport_scan",
    );
    if (!asset) throw new Error("Missing passport document asset");
    ...
    await expect(
      options.downloadDocument?.(asset, {
        applicant: submission.applicants[0]!,
        applicantIndex: 0,
        exportDate: "2026-07-28",
        submission,
