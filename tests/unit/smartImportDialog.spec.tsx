...
describe("SmartImportDialog", () => {
  test("traps Tab focus and restores the opener after Escape", async () => {
    function Harness() {
      const [open, setOpen] = useState(false);
      return (
        <>
          <button type="button" onClick={() => setOpen(true)}>
            Открыть умный импорт
          </button>
          <SmartImportDialog
            applicantKey="applicant-1"
            currentValues={{}}
    ...
    render(<Harness />);
    const opener = screen.getByRole("button", { name: "Открыть умный импорт" });
    opener.focus();
    fireEvent.click(opener);

    const close = await screen.findByRole("button", { name: "Закрыть умный импорт" });
    const dialog = screen.getByRole("dialog", { name: "Умный импорт" });
    ...
    if (!last) throw new Error("Expected a final Smart Import focus target.");
  ...
  test("processes pasted text, clears it, and shows sanitized review rows", async () => {
    ...
    expect(screen.getAllByText("Раздел: Контакты")).toHaveLength(2);
    expect(screen.getByText("Раздел: Работа")).toBeInTheDocument();
    const review = screen.getByRole("region", { name: "Проверка распознанных полей" });
  ...
  test("ignores a late text result after cancellation", async () => {
    ...
    await waitFor(() =>
      expect(
        screen.getByRole("button", { name: "Закрыть умный импорт" }),
      ).toHaveFocus(),
    );
    focus.mockClear();
    ...
    expect(
      screen.queryByRole("region", { name: "Проверка распознанных полей" }),
    ).not.toBeInTheDocument();
    expect(focus).not.toHaveBeenCalled();
    focus.mockRestore();
  });

  test("keeps the source view without moving focus when extraction fails", async () => {
    const focus = vi.spyOn(HTMLElement.prototype, "focus");
    const extraction: SmartImportDialogExtraction = {
      fromFile: vi.fn(async () => parsed),
      fromText: vi.fn(async () => {
        throw new Error("Не удалось разобрать текст.");
      }),
    };
    renderDialog({ extraction });
    await waitFor(() =>
    ...
    focus.mockClear();
    const textbox = screen.getByLabelText("Вставить текст");
    fireEvent.change(textbox, { target: { value: "некорректный источник" } });
    fireEvent.click(screen.getByRole("button", { name: "Распознать текст" }));
