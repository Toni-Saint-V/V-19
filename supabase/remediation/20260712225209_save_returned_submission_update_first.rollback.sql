...
--
-- If 20260712225209_save_returned_submission_update_first.sql must be rolled
-- back after promotion, create a new timestamped migration with
-- `supabase migration new restore_submission_draft_upsert`, copy this file
-- into that migration, obtain explicit owner approval, and apply the new
-- migration through the normal promotion workflow.
--
-- The function body below is the exact pre-change body from
-- 20260710000300_persist_handoff_applicant_projection.sql. The statements
-- after it preserve least privilege and fail unless the SECURITY INVOKER
-- execution contract remains intact.
...
begin
  ...
  if function_record.prosecdef is distinct from false
    or not coalesce(function_record.proconfig, '{}'::text[])
      @> array['search_path=public']
  then
    raise exception 'Submission draft helper SECURITY INVOKER contract was not restored';
  end if;
  ...
  if has_function_privilege(
    ...
    raise exception 'Submission draft helper ACL contract was not restored';
  end if;
end;
$$;
