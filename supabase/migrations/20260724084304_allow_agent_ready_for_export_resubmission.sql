-- Canonical T2 permits an agent-owned ready_for_export package to return to
-- submitted_for_review. The persisted legacy states for that transition are
-- ready_for_excel -> waiting_review. Keep the exception narrow: it may clear
-- review/export readiness, but it must not reopen an exported package or
-- weaken the normal agent mutation boundary.
...
begin
  ...
  if old.status = 'ready_for_excel' and new.status = 'waiting_review' then
    if old.accepted_at is null
      or old.exported_at is not null
      or new.type is distinct from old.type
      or new.title is distinct from old.title
      or new.country is distinct from old.country
      or new.city is distinct from old.city
      or new.travel_date is distinct from old.travel_date
      or new.trip_date_from is distinct from old.trip_date_from
      or new.trip_date_to is distinct from old.trip_date_to
      or new.priority is distinct from old.priority
      or new.readiness_percent is distinct from old.readiness_percent
      or new.appointment_status is distinct from old.appointment_status
      or new.appointment_status <> 'not_started'
      or new.review_started_at is not null
      or new.accepted_at is not null
      or new.exported_at is not null
    then
      raise exception 'Accepted resubmission must clear review readiness without changing export completion'
        using errcode = '42501';
    end if;
...
-- The same transaction must return accepted media to admin review. The
-- submission guard above is the only writer of this transaction-local flag.
-- A direct media update, a cross-submission update, or any content mutation
-- remains denied.
...
declare
  actor_role public.profile_role := app_private.current_profile_role();
  accepted_resubmission_submission_id text :=
    current_setting('app.visaflow_accepted_resubmission_submission_id', true);
...
begin
  ...
  if actor_role = 'agent' then
    if tg_op = 'INSERT' then
    ...
    elsif accepted_resubmission_submission_id = new.submission_id
      ...
      and exists (
        ...
        where submission.id = new.submission_id
          ...
          and submission.status = 'waiting_review'
      )
    then
...
comment on function app_private.enforce_submission_agent_mutation() is
  'Allows only the canonical agent-owned ready_for_excel -> waiting_review resubmission to clear review readiness; all other operator/export state remains server-owned.';

comment on function app_private.enforce_media_asset_review_boundary() is
  'Keeps media review state server-owned and atomically resets accepted media only during a canonical accepted-package resubmission.';
