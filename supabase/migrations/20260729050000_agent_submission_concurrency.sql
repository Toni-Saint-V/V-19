...
-- Keep the runtime issue identity bound to the durable correction row. This
-- mirrors the client stableUuid() algorithm so an Agent cannot swap a cockpit
-- issue while sending an otherwise valid correction UUID.
...
declare
  code_point bigint;
  hash_a bigint := 2166136261;
  hash_b bigint := 16777619;
  hash_hex text;
  position_index integer;
  seed text := 'correction:' || submission_id || ':' || issue_id;
  xor_value bigint;
begin
  if nullif(submission_id, '') is null or nullif(issue_id, '') is null then
    return null;
  end if;

  for position_index in 1..char_length(seed)
  loop
    code_point := ascii(substring(seed from position_index for 1));
    xor_value := hash_a # code_point;
    hash_a := mod(
      xor_value::numeric * 16777619::numeric,
      4294967296::numeric
    )::bigint;

    xor_value := hash_b # mod(
      (code_point + hash_a)::numeric,
      4294967296::numeric
    )::bigint;
    hash_b := mod(
      xor_value::numeric * 2246822507::numeric,
      4294967296::numeric
    )::bigint;
  end loop;

  hash_hex :=
    lpad(to_hex(hash_a), 8, '0')
    || lpad(to_hex(hash_b), 8, '0')
    || lpad(to_hex(hash_a # hash_b), 8, '0')
    || lpad(
      to_hex(
        mod((hash_a + hash_b)::numeric, 4294967296::numeric)::bigint
      ),
      8,
      '0'
    );
  ...
  return (
    ...
    || '-4'
    || substring(hash_hex from 14 for 3)
    || '-8'
    || substring(hash_hex from 18 for 3)
    || '-'
...
revoke all on function app_private.agent_issue_correction_id(text, text)
  from public, anon, authenticated;
grant execute on function app_private.agent_issue_correction_id(text, text)
  to authenticated;

create or replace function app_private.agent_correction_created_by_admin(
  correction_id uuid,
  submission_id text
)
returns boolean
...
as $function$
  select exists (
    ...
    join public.profiles as creator_profile
      on creator_profile.id = correction.created_by
      and creator_profile.role = 'admin'
    join public.submissions as submission
      on submission.id = correction.submission_id
    where correction.id = $1
      and correction.submission_id = $2
      and (
        submission.agent_id = auth.uid()
        or app_private.current_profile_role() = 'admin'
      )
  )
$function$;

revoke all on function app_private.agent_correction_created_by_admin(uuid, text)
  from public, anon, authenticated;
grant execute on function app_private.agent_correction_created_by_admin(uuid, text)
  to authenticated;
...
declare
  ...
  normalized_corrections jsonb := '[]'::jsonb;
...
begin
  ...
  snapshot_issues := snapshot_submission -> 'issues';
  ...
  if jsonb_typeof(submission_record.family_intelligence)
    ...
    or jsonb_typeof(snapshot_issues) is distinct from 'array'
  ...
  if jsonb_typeof(payload -> 'corrections') is distinct from 'array' then
    raise exception 'Agent corrections payload must be an array'
      using errcode = '23514';
  end if;

  if jsonb_array_length(snapshot_issues)
      <> jsonb_array_length(payload -> 'corrections')
    or exists (
      select 1
      from jsonb_array_elements(snapshot_issues) as snapshot_issue(value)
      where nullif(
        btrim(coalesce(snapshot_issue.value ->> 'id', '')),
        ''
      ) is null
        or snapshot_issue.value ->> 'createdBy' not in ('admin', 'system')
        or nullif(
          btrim(coalesce(snapshot_issue.value ->> 'createdAt', '')),
          ''
        ) is null
    )
    or exists (
      select 1
      from jsonb_array_elements(snapshot_issues) as snapshot_issue(value)
      group by snapshot_issue.value ->> 'id'
      having count(*) > 1
    )
    or exists (
      select 1
      from jsonb_array_elements(payload -> 'corrections')
        as correction_payload(value)
      where nullif(
        btrim(coalesce(correction_payload.value ->> 'id', '')),
        ''
      ) is null
    )
    or exists (
      ...
      from jsonb_array_elements(payload -> 'corrections')
        as correction_payload(value)
      group by correction_payload.value ->> 'id'
      having count(*) > 1
    )
    or exists (
      select 1
      from jsonb_array_elements(snapshot_issues) with ordinality
        as snapshot_issue(value, position_index)
      join jsonb_array_elements(payload -> 'corrections') with ordinality
        as correction_payload(value, position_index)
        using (position_index)
      where correction_payload.value ->> 'id' is distinct from
          app_private.agent_issue_correction_id(
            submission_record.id,
            snapshot_issue.value ->> 'id'
          )::text
        or correction_payload.value ->> 'submission_id'
          is distinct from submission_record.id
        or correction_payload.value ->> 'applicant_id'
          is distinct from snapshot_issue.value #>> '{target,applicantId}'
        or correction_payload.value ->> 'scope' is distinct from (
          case snapshot_issue.value ->> 'type'
            when 'field' then 'field'
            when 'section' then 'field'
            when 'file' then 'media'
            when 'media' then 'media'
            else null
          end
        )
        or correction_payload.value ->> 'field_key' is distinct from (
          case
            when snapshot_issue.value ->> 'type' in ('field', 'section')
            then snapshot_issue.value #>> '{target,field}'
            else null
          end
        )
        or correction_payload.value ->> 'media_type' is distinct from (
          case
            when snapshot_issue.value ->> 'type' in ('file', 'media')
            then snapshot_issue.value #>> '{target,fileType}'
            else null
          end
        )
        or correction_payload.value ->> 'reason' is distinct from (
          coalesce(snapshot_issue.value ->> 'reason', '')
          || case
            when coalesce(snapshot_issue.value ->> 'comment', '') = ''
              then ''
            else ' — ' || (snapshot_issue.value ->> 'comment')
          end
        )
        or correction_payload.value ->> 'severity' is distinct from (
          case snapshot_issue.value ->> 'severity'
            when 'blocker' then 'blocking'
            when 'warning' then 'note'
            when 'info' then 'note'
            else null
          end
        )
        or correction_payload.value ->> 'status' is distinct from (
          case snapshot_issue.value ->> 'status'
            when 'open' then 'open'
            when 'fixed_by_agent' then 'fixed'
            when 'closed_by_admin' then 'closed'
            else null
          end
        )
    )
  then
    raise exception 'Agent cockpit issues diverge from the corrections projection'
      using errcode = '23514';
  end if;

  if jsonb_typeof(final_history) is distinct from 'array'
    or exists (
      select 1
  ...
  if existing_submission.id is null then
    if jsonb_array_length(snapshot_issues) <> 0
      or jsonb_array_length(payload -> 'corrections') <> 0
    then
      raise exception 'New Agent submissions cannot create review issues'
        using errcode = '42501';
    end if;
  else
    select count(*)::integer
    into durable_correction_count
    from public.corrections as correction
    where correction.submission_id = submission_record.id;

    existing_snapshot_issues :=
      existing_submission.family_intelligence
        #> '{v19CockpitSnapshot,submission,issues}';

    if durable_correction_count
        <> jsonb_array_length(payload -> 'corrections')
    then
      raise exception 'Agent cannot add or remove Admin review issues'
        using errcode = '42501';
    end if;

    if durable_correction_count > 0
      and jsonb_typeof(existing_snapshot_issues) is distinct from 'array'
    then
      raise exception 'Agent review issues lack a trusted durable snapshot identity'
        using errcode = '42501';
    end if;

    if jsonb_typeof(existing_snapshot_issues) = 'array'
      and (
        jsonb_array_length(existing_snapshot_issues)
          <> jsonb_array_length(snapshot_issues)
        or exists (
          select 1
          from jsonb_array_elements(existing_snapshot_issues)
            as existing_issue(value)
          group by existing_issue.value ->> 'id'
          having count(*) > 1
        )
        or exists (
          select 1
          from jsonb_array_elements(snapshot_issues) as incoming_issue(value)
          left join jsonb_array_elements(existing_snapshot_issues)
            as existing_issue(value)
            on existing_issue.value ->> 'id' =
              incoming_issue.value ->> 'id'
          where existing_issue.value is null
            or (incoming_issue.value - 'status')
              is distinct from (existing_issue.value - 'status')
            or not (
              incoming_issue.value ->> 'status'
                = existing_issue.value ->> 'status'
              or (
                existing_issue.value ->> 'status' = 'open'
                and incoming_issue.value ->> 'status' = 'fixed_by_agent'
            ...
            or (
              ...
              and incoming_issue.value ->> 'status' = 'fixed_by_agent'
              and not (
                case incoming_issue.value ->> 'type'
                  when 'field' then exists (
                    ...
                    where snapshot_applicant.value ->> 'id' =
                        incoming_issue.value #>> '{target,applicantId}'
                      and snapshot_field.value ->> 'id' =
                        incoming_issue.value #>> '{target,field}'
                      and nullif(
                        btrim(
                          coalesce(snapshot_field.value ->> 'value', '')
                        ),
                        ''
                      ) is not null
                      and btrim(
                        ...
                        coalesce(existing_issue.value ->> 'snapshot', '')
                      )
                  )
                  ...
                  when 'file' then exists (
                    select 1
                    from jsonb_array_elements(snapshot_files)
                      as snapshot_file(value)
                    join jsonb_to_recordset(payload -> 'media_assets')
                      as incoming_media (
                        applicant_id text,
                        submission_id text,
                        type text,
                      ...
                      on incoming_media.applicant_id =
                        snapshot_file.value ->> 'applicantId'
                      and incoming_media.type =
                        snapshot_file.value ->> 'type'
                    left join public.media_assets as durable_media
                      on durable_media.submission_id = submission_record.id
                      and durable_media.applicant_id =
                        incoming_media.applicant_id
                      and durable_media.type::text = incoming_media.type
                    where snapshot_file.value ->> 'applicantId' =
                        incoming_issue.value #>> '{target,applicantId}'
                      and snapshot_file.value ->> 'type' =
                        incoming_issue.value #>> '{target,fileType}'
                      and snapshot_file.value ->> 'uploadStatus' = 'uploaded'
                      and snapshot_file.value ->> 'status'
                        not in ('missing', 'needs_replacement')
                      and incoming_media.submission_id =
                        submission_record.id
                      and incoming_media.upload_status = 'uploaded'
                      and incoming_media.review_status = 'not_reviewed'
                      and incoming_media.storage_bucket = 'submission-media'
                      and nullif(
                        btrim(coalesce(incoming_media.generated_file_name, '')),
                        ''
                      ) is not null
                      and incoming_media.storage_path !~ '(^/|//|\.\.)'
                      and split_part(incoming_media.storage_path, '/', 1) =
                        'submissions'
                      and split_part(incoming_media.storage_path, '/', 2) =
                        submission_record.id
                      and split_part(incoming_media.storage_path, '/', 3) =
                        'applicants'
                      and split_part(incoming_media.storage_path, '/', 4) =
                        incoming_media.applicant_id
                      and split_part(incoming_media.storage_path, '/', 5) =
                        incoming_media.type
                      and split_part(incoming_media.storage_path, '/', 6) =
                        incoming_media.generated_file_name
                      and split_part(incoming_media.storage_path, '/', 7) = ''
                      and exists (
                        select 1
                        from storage.objects as stored_object
                        where stored_object.bucket_id =
                            incoming_media.storage_bucket
                          and stored_object.name =
                            incoming_media.storage_path
                      )
                      and (
                        durable_media.id is null
                        or durable_media.storage_path is distinct from
                          incoming_media.storage_path
                      )
                  )
    ...
    then
      raise exception 'Agent cannot rewrite Admin issue identity or immutable evidence'
        using errcode = '42501';
    end if;

    if exists (
      select 1
      from jsonb_to_recordset(payload -> 'corrections')
        as incoming_correction (
          id uuid,
          submission_id text,
          applicant_id text,
          scope text,
      ...
      left join public.corrections as durable_correction
        on durable_correction.id = incoming_correction.id
        and durable_correction.submission_id = submission_record.id
      where durable_correction.id is null
        or incoming_correction.submission_id
          is distinct from durable_correction.submission_id
        or incoming_correction.applicant_id
          is distinct from durable_correction.applicant_id
        or incoming_correction.scope
          is distinct from durable_correction.scope
        or incoming_correction.field_key
          is distinct from durable_correction.field_key
        or incoming_correction.media_type
          is distinct from durable_correction.media_type::text
        or incoming_correction.reason
          is distinct from durable_correction.reason
        or incoming_correction.severity
          is distinct from durable_correction.severity
        or not app_private.agent_correction_created_by_admin(
          durable_correction.id,
          submission_record.id
        )
        or (
          durable_correction.status = 'open'
          and durable_correction.fixed_at is not null
        )
        or (
          durable_correction.status in ('fixed', 'closed')
          and durable_correction.fixed_at is null
        )
        or not (
          incoming_correction.status = durable_correction.status
          or (
            durable_correction.status = 'open'
            and incoming_correction.status = 'fixed'
            and existing_submission.status in (
              'returned',
              'ready_for_review'
            )
      ...
      raise exception 'Agent correction mutation violates Admin issue ownership'
        using errcode = '42501';
    end if;
    ...
    select coalesce(
      jsonb_agg(
        jsonb_build_object(
          'id', durable_correction.id,
          'submission_id', durable_correction.submission_id,
          'applicant_id', durable_correction.applicant_id,
          'scope', durable_correction.scope,
          'field_key', durable_correction.field_key,
          'media_type', durable_correction.media_type,
          'reason', durable_correction.reason,
          'severity', durable_correction.severity,
          'status', incoming_correction.value ->> 'status',
          'created_by', durable_correction.created_by,
          'created_at', durable_correction.created_at,
          'fixed_at', case
            when durable_correction.status = 'open'
              and incoming_correction.value ->> 'status' = 'fixed'
            then statement_timestamp()
            else durable_correction.fixed_at
          end
        )
        order by incoming_correction.position_index
      ),
      '[]'::jsonb
    )
    into normalized_corrections
    from jsonb_array_elements(payload -> 'corrections') with ordinality
      as incoming_correction(value, position_index)
    join public.corrections as durable_correction
      on durable_correction.id =
        (incoming_correction.value ->> 'id')::uuid
      and durable_correction.submission_id = submission_record.id;
...
declare
  ...
  correction_creator_function_oid oid := to_regprocedure(
    'app_private.agent_correction_created_by_admin(uuid,text)'
  )::oid;
  issue_identity_function_oid oid := to_regprocedure(
    'app_private.agent_issue_correction_id(text,text)'
  )::oid;
...
begin
  if function_oid is null
    or correction_creator_function_oid is null
    or issue_identity_function_oid is null
    or receipt_table_oid is null
  then
  ...
  if not exists (
    select 1
    from pg_catalog.pg_proc as procedure
    where procedure.oid = correction_creator_function_oid
      and procedure.prosecdef
  ) then
    raise exception 'Admin correction creator attestation must be SECURITY DEFINER';
  end if;

  if has_function_privilege(
    'anon',
    correction_creator_function_oid,
    'EXECUTE'
  ) or exists (
    select 1
    from pg_catalog.pg_proc as procedure
    ...
    raise exception 'Admin correction creator attestation is publicly executable';
  end if;
  ...
  if not has_function_privilege(
    ...
    raise exception 'Authenticated correction creator attestation is missing';
  end if;
  ...
  if not exists (
    ...
    raise exception 'Admin correction creator attestation has an unexpected search_path';
  end if;
