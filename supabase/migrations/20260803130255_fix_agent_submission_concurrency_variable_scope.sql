-- PostgreSQL does not implicitly expose PL/pgSQL variables through the
-- function-name qualifier inside embedded SQL statements. Reinstall the exact
-- reviewed CAS writer with an explicit block label so the qualified references
-- resolve deterministically without trusting or preserving remote function drift.
begin;
