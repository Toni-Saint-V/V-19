...
export function Drawer({ isOpen, onClose, submissionId, onOpenQuestionnaire }: DrawerProps) {
  ...
  const getFooterActions = () => {
    ...
    if (data.status === 'returned') {
      return (
        <button className="flex-1 sm:flex-none h-11 px-8 bg-[#24242a] hover:bg-[#2a2b32] text-white font-medium text-[14px] rounded-xl shadow-[0_0_28px_rgba(111,100,255,0.14)] transition-colors flex items-center justify-center gap-2">
    ...
    if (data.status === 'in_progress') {
      return (
        <button className="flex-1 sm:flex-none h-11 px-8 bg-[#6f64ff] hover:bg-[#4855d4] text-white font-medium text-[14px] rounded-xl shadow-[0_0_20px_rgba(58,69,180,0.3)] transition-colors flex items-center justify-center gap-2">
