...
export function AdminReviewDrawer({
  ...
  const handlePublishReturnedPdfHandoff = () => {
    if (!submissionId || !canPublishReturnedPdfHandoff) return;
    void onPublishReturnedPdfHandoff?.(submissionId);
    void bridge.onPublishReturnedPdfHandoff?.(submissionId);
    emitVisaflowUiEvent(bridge, { type: 'returned-pdf-handoff.publish', submissionId });
  };
          ...
          >
            ...
            <footer className="p-4 lg:px-8 lg:py-5 border-t border-white/10 bg-[#111113]/90 backdrop-blur-md shrink-0 flex justify-end gap-3 pb-[max(16px,env(safe-area-inset-bottom))]">
              ...
              <button
                ...
                title={canPublishReturnedPdfHandoff ? 'Передать опубликованный PDF-пакет агенту' : returnedPdfHandoffReason}
                className="h-11 px-5 bg-white/[0.045] hover:bg-white/[0.08] disabled:bg-white/5 disabled:text-white/30 disabled:cursor-not-allowed text-white/70 font-medium text-[13px] rounded-xl transition-colors border border-white/10"
              >
                Передать PDF агенту
              </button>
