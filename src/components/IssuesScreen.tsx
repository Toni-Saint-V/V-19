...
const mockIssues: Issue[] = [
  {
    ...
    description:
      "В анкете указано 12.05.1985, а в загруженном PDF скане паспорта — 15.05.1985. Требуется ручное подтверждение менеджера.",
  ...
  },
  {
    ...
    description:
      "Для поездки в зону Schengen требуется выписка с банковского счета с остатком не менее 300 000 ₽.",
  ...
  },
  {
    ...
    description:
      "Паспорт истекает через 6.5 месяцев. Консульство Китая рекомендует иметь запас минимум 6 месяцев. Проходит по границе.",
      ...
      <div className="grid grid-cols-2 gap-3 lg:gap-4">
        <div className="p-4 lg:p-5 rounded-2xl bg-[#a35f69]/5 border border-[#5b2b32]/45 shadow-[0_4px_24px_rgba(0,0,0,0.12)] flex flex-col justify-between h-[100px] relative overflow-hidden group">
          ...
          <div className="flex items-center justify-between relative z-10">
            <span className="text-[12px] font-medium text-[#d59aa3] uppercase tracking-wide">
              Критичные ошибки
            </span>
        ...
        <div className="p-4 lg:p-5 rounded-2xl bg-white/[0.035] border border-white/10 flex flex-col justify-between h-[100px] relative overflow-hidden group">
          ...
          <div className="flex items-center justify-between relative z-10">
            <span className="text-[12px] font-medium text-white/62 uppercase tracking-wide">
              Предупреждения
            </span>
      ...
      <div className="space-y-3">
        {issues.map((issue) => (
          ...
          >
            {/* Severity Indicator */}
            <div
              className={`w-10 h-10 shrink-0 rounded-xl flex items-center justify-center border shadow-inner mt-1
            ...
            <div className="flex-1 min-w-0 flex flex-col justify-center">
              <div className="flex flex-wrap items-center gap-2 text-[11px] text-white/40 mb-1.5 font-medium">
                ...
                <span className="flex items-center gap-1.5">
                  <User className="w-3 h-3" /> {issue.applicant}
                </span>
                <span className="w-1 h-1 rounded-full bg-white/20" />
                <span>{issue.date}</span>
              </div>

              <h3 className="text-[15px] font-semibold text-white mb-1.5 group-hover:text-[#b8baff] transition-colors">
                {issue.title}
              </h3>
      ...
      {issues.length === 0 && (
        <div className="flex flex-col items-center justify-center py-20 text-center bg-[#161617] rounded-2xl border border-[#242529] border-dashed">
          ...
          <p className="text-[13px] text-white/50 max-w-xs leading-relaxed">
            Система работает стабильно. Все документы и анкеты проходят проверки без
            замечаний.
          </p>
        </div>
      )}
