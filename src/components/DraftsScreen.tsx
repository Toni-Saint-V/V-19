...
import { documentCellsForSubmission } from './v19BusinessScreenAdapter';
import { 
...
function runtimeDraftsFromSubmissions(submissions?: Submission[]): DraftSubmission[] {
  if (!submissions?.length) return mockDrafts;
  ...
  return submissions.map((submission) => ({
    ...
    deadline: submission.status === 'returned' ? 'Требует исправлений' : submission.status === 'submitted_for_review' ? 'На проверке' : submission.status === 'ready_for_export' ? 'Готово к выгрузке' : 'В работе',
    applicants: submission.applicants.map((applicant) => {
      const cells = documentCellsForSubmission(submission).filter((cell) => cell.applicantId === applicant.id);
      const passport = cells.find((cell) => cell.key.endsWith(':passport_scan'))?.status ?? 'missing';
      const selfie = cells.find((cell) => cell.key.endsWith(':selfie'))?.status ?? 'missing';
      const selfie2 = cells.find((cell) => cell.key.endsWith(':selfie_2'))?.status ?? 'missing';
      const questionnaire: DocStatus = applicant.questionnaireStatus === 'complete' ? 'verified' : applicant.questionnaireStatus === 'needs_fix' ? 'error' : applicant.questionnaireStatus === 'partial' ? 'processing' : 'missing';
      ...
      return {
        ...
        docs: {
          ...
          selfie,
          financial: selfie2,
          booking: submission.tripDateFrom && submission.tripDateTo ? 'verified' : 'missing',
          insurance: submission.completeness.files >= 100 ? 'verified' : 'processing',
          questionnaire,
        },
      };
    }),
...
export function DraftsScreen({ onOpenDrawer, submissions }: DraftsScreenProps) {
  const visibleDrafts = useMemo(() => runtimeDraftsFromSubmissions(submissions), [submissions]);
