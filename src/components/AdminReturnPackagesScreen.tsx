...
export function AdminReturnPackagesScreen() {
  ...
  const ensurePackage = async (
    ...
    if (started.applicantCount !== targetGroup.applicants.length) {
      throw new Error("Состав выгрузки изменился. Обновите страницу до загрузки PDF.");
    }
  ...
  const publish = async () => {
    ...
    try {
      ...
      setError(
        caught instanceof Error ? caught.message : "Не удалось передать пакет агенту.",
      );
    } finally {
      setBusySlot("");
    }
  ...
  if (groupsError) {
    return (
      <section className="flex min-h-[420px] flex-col items-center justify-center rounded-2xl border border-[#513036] bg-[#281c20] p-8 text-center">
        <h2 className="m-0 text-[18px] font-semibold text-white">
          Не удалось загрузить возвратные пакеты
        </h2>
        <p className="m-0 mt-2 max-w-md text-[13px] leading-5 text-[#ffbdc3]">
          {groupsError}
        </p>
      </section>
    );
  ...
  if (!groups.length) {
    return (
      <section className="flex min-h-[420px] flex-col items-center justify-center rounded-2xl border border-dashed border-[#242529] bg-[#161617] p-8 text-center">
        ...
        <h2 className="m-0 text-[18px] font-semibold text-white">
          Нет выгруженных пакетов
        </h2>
        <p className="m-0 mt-2 max-w-md text-[13px] leading-5 text-white/52">
          После городской выгрузки здесь появятся группы для возврата готовых анкет
          агентам.
        </p>
      </section>
    );
  ...
  return (
    <section
      ...
      data-testid="admin-return-packages-screen"
    >
      ...
      {activeGroup ? (
        <div className="grid gap-5">
          <header className="flex flex-col gap-3 rounded-2xl border border-[#242529] bg-[#161617] p-5 md:flex-row md:items-center md:justify-between">
            <div>
              ...
              <p className="m-0 mt-2 text-[13px] text-white/54">
                Один PDF-список на {activeGroup.applicants.length} туристов и одна
                готовая форма на каждого.
              </p>
            </div>
              ...
              <label className="inline-flex h-10 cursor-pointer items-center justify-center gap-2 rounded-[10px] border border-[#4450c5] bg-[#3a45b4] px-3 text-[12px] font-semibold text-white transition-colors hover:bg-[#4855d4]">
                ...
                <input
                  ...
                  onChange={(event) => {
                    ...
                    if (file)
                      void upload("list", file, { artifactKind: "agent_list_pdf" });
                  }}
                />
              </label>
            </div>
                  ...
                  >
                    <div className="min-w-0">
                      ...
                      <span className="mt-1 block text-[12px] text-white/48">
                        {applicant.submissionType === "family"
                          ? "Семья"
                          : "Одиночная подача"}{" "}
                        · {applicant.submissionTitle}
                      </span>
                    </div>
                    <label className="inline-flex h-10 shrink-0 cursor-pointer items-center justify-center gap-2 rounded-[10px] border border-[#2e2f34] bg-[#1e1e21] px-3 text-[12px] font-semibold text-white/80 transition-colors hover:bg-[#27272b]">
                      ...
                      <input
                        ...
                        className="sr-only"
                        disabled={
                          Boolean(busySlot) || startedPackage?.status === "published"
                        }
                        type="file"
                        onChange={(event) => {
                          const file = event.currentTarget.files?.[0];
          ...
          {error ? (
            <p className="m-0 rounded-xl border border-[#513036] bg-[#281c20] px-4 py-3 text-[13px] text-[#ffbdc3]">
              {error}
            </p>
          ) : null}
          {notice ? (
            <p className="m-0 rounded-xl border border-[#244238] bg-[#14251f] px-4 py-3 text-[13px] text-[#8fe7c1]">
              {notice}
            </p>
          ) : null}
          ...
          <footer className="flex flex-col gap-3 rounded-2xl border border-[#242529] bg-[#161617] p-4 sm:flex-row sm:items-center sm:justify-between">
            <span className="text-[12px] text-white/50">
              {hasList ? "Список загружен" : "Нужен PDF-список"} ·{" "}
              {
                artifacts.filter(
                  (artifact) => artifact.artifactKind === "visa_application_pdf",
                ).length
              }
              /{activeGroup.applicants.length} анкет
            </span>
            <div className="flex flex-wrap items-center gap-2">
              ...
              <button
                ...
                disabled={
                  !canPublish ||
                  Boolean(busySlot) ||
                  startedPackage?.status === "published"
                }
                type="button"
