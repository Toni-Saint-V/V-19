...
const familyDataStyleBaseId = 52;
const familyStyleVariantCount = 4;

export function createExportWorkbookBytes(
...
export function createExportWorkbookBlob(
  ...
  const zip = createExportWorkbookBytes(workbookRows, options);
...
function stylesXml(): string {
  ...
  const familyXfs = familyFillPalette
    ...
    .join("");
  const fillsCount = 3 + familyFillPalette.length;
  const cellXfsCount = familyDataStyleBaseId + familyFillPalette.length * familyStyleVariantCount;

  return `<?xml version="1.0" encoding="UTF-8" standalone="yes"?><styleSheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"><numFmts count="1"><numFmt numFmtId="164" formatCode="yyyy\\-mm\\-dd;@"/></numFmts><fonts count="1"><font><sz val="11"/><name val="Calibri"/><family val="2"/><scheme val="minor"/></font></fonts><fills count="${fillsCount}"><fill><patternFill patternType="none"/></fill><fill><patternFill patternType="gray125"/></fill>${headerFill}${familyFills}</fills><borders count="2"><border><left/><right/><top/><bottom/><diagonal/></border><border><left style="thin"><color indexed="64"/></left><right style="thin"><color indexed="64"/></right><top style="thin"><color indexed="64"/></top><bottom style="thin"><color indexed="64"/></bottom><diagonal/></border></borders><cellStyleXfs count="1"><xf numFmtId="0" fontId="0" fillId="0" borderId="0"/></cellStyleXfs><cellXfs count="${cellXfsCount}">${baseXfs}${familyXfs}</cellXfs><cellStyles count="1"><cellStyle name="Normal" xfId="0" builtinId="0"/></cellStyles></styleSheet>`;
}
...
function dataRowXml(
  ...
  const cells = Array.from({ length: templateVisibleColumnCount }, (_, index) => {
    ...
    if (index >= templateColumnCount) {
      return `<c r="${column}${rowNumber}" s="${rowFill ? familyStyleId(column, rowFill) : 33}"/>`;
    }
...
function familyStyleId(column: string, rowFill: ExportWorkbookRowFill) {
  const base = familyDataStyleBaseId + familyPaletteIndex(rowFill) * familyStyleVariantCount;
  if (dateColumnNames.has(column)) return base + 2;
  if (column === "E" || column === "AP" || column === "AY") {
    return base + 1;
  }
  if (column === "BC") return base + 3;
  return base;
}

function familyPaletteIndex(rowFill: ExportWorkbookRowFill): number {
  const index = Number(rowFill.match(/family-(\d+)/)?.[1] ?? 1) - 1;
  if (!Number.isFinite(index) || index < 0) return 0;
  return index % familyFillPalette.length;
}
