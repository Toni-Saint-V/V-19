...
export function getExportWarnings(submissions: Submission[]): ExportBlocker[] {
  ...
  if (cities.size > 1) {
    warnings.push({
      reason: "Выборка будет автоматически разделена на отдельные Excel по городам.",
    });
  }
  ...
  if (ownerAgentIds.size > 1) {
    warnings.push({
      reason:
        "В выборке подачи разных агентов. Excel доступен, PDF-пакеты вернутся своим агентам.",
    });
  }
