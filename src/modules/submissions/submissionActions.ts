...
export function approveQuestionnaireFieldForAdmin(
  ...
  const hasOpenIssue = submission.issues.some((issue) => {
    if (issue.status !== "open" || issue.target.applicantId !== input.applicantId) {
      return false;
    }
...
export function addPreciseAdminIssue(
  ...
  if (!applicant) return submission;
  const targetIdentity = resolveQuestionnaireIssueInputIdentity(applicant, issueInput);
  if (!issueInput.fileType && !targetIdentity) return submission;
  ...
  const withTargetFlag = newIssue.target.fileType
    ...
    : flagQuestionnaireIssueField(submission, newIssue, newIssue.reason);
