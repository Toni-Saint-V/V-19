...
function AdminAiAssistancePanel({
  ...
  const reviewItems = state.review?.adminReviewChecklist?.length
  ...
  return (
    <section className="admin-ai-assist" aria-label="AI-помощник администратора">
      ...
      {state.status === "loading" ? (
        <p role="status">
          Запрос идет через серверный ai-helper. Действия не выполняются.
        </p>
      ) : null}
      ...
      {state.status === "ready" ? (
        <div className="admin-ai-assist-grid">
          ...
          <section>
            <h3>Ограничение</h3>
            <p>
              Требует проверки администратором. Принятие и выгрузка остаются ручными
              действиями.
            </p>
          </section>
        </div>
