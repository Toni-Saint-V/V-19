...
const passportReviewFieldIds = new Set<SmartImportFieldId>([
  "passport-type",
  "passport-no",
  "passport-issue-date",
  "passport-expiry-date",
...
const fieldMetadata: Record<SmartImportFieldId, { label: string; sectionId: string }> =
  {
    "appointment-city": { label: "Город подачи", sectionId: "appointment" },
    "desired-date-1": { label: "Желаемая дата 1", sectionId: "appointment" },
    "desired-date-2": { label: "Желаемая дата 2", sectionId: "appointment" },
    ...
    "guardian-info": { label: "Данные опекуна", sectionId: "personal" },
    "passport-type": { label: "Тип паспорта", sectionId: "passport" },
    ...
    "company-org-details": { label: "Данные организации", sectionId: "hotel" },
    "company-contact-person": { label: "Контактное лицо", sectionId: "hotel" },
    "company-phone": { label: "Телефон организации", sectionId: "hotel" },
  };
  ...
  return {
    ...
    items: input.parsed.candidates.map((candidate, index) => {
      ...
      return {
        ...
        selectedByDefault:
          !candidate.manualReview && status === "new" && candidate.confidence !== "low",
        status,
      };
    }),
...
function classifySmartImportDocument(source: string): SmartImportDocumentKind {
  ...
  // A dense labelled questionnaire may contain hotel, employer and travel
  // values together. It must remain a form so role-specific labels are not
  // re-routed by a single incidental document keyword.
  if (labelledLineCount >= 7) return "filled_form";
  if (
...
function collectGenericContactCandidates(
  ...
  // Passport numbers often contain a phone-like run of digits. A labelled
  // passport field is already handled by the questionnaire parser, so do not
  // let its number become an unlabelled contact candidate.
  const sourceHasLabelledPassportNumber = parseQuestionnaireDataText(source).some(
    (field) => field.fieldId === "passport-no",
  );
  const phone = sourceHasLabelledPassportNumber ? undefined : findPhone(source);
  if (phone) {
    const fieldId =
      documentKind === "booking" || documentKind === "invitation"
        ? "hotel-contact"
...
function collectBookingCandidates(source: string, candidates: CandidateInput[]) {
  ...
  const lines = source
    ...
    .filter(Boolean);
  const hotelLine = lines.find((line) =>
  ...
  if (postalCityCountry) {
    ...
    addCandidate(
      ...
      candidate(
        "hotel-country",
        normalizeCountry(postalCityCountry[3] ?? ""),
        "high",
        90,
      ),
    );
  ...
  return [...byFieldId.values()]
    .map(
      ({
        ...
        fieldId,
        label,
        manualReview: passportReviewFieldIds.has(fieldId) || undefined,
        sectionId,
        sourceKind,
        value,
      }),
