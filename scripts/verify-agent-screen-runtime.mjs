...
verifyRuntimeStyleTopology();
verifyUnifiedSidebarContract();
verifySubmissionVisualBypassContract();
verifyV19SemanticStyleContract();

if (failures.length > 0) {
  console.error("Agent screen runtime verification failed:");
  for (const failure of failures) console.error(`- ${failure}`);
  process.exit(1);
}

console.log("Agent screen runtime verification passed");
...
function verifyCssTokenContract() {
  ...
  for (const styleSource of styleSources) {
    let parsedStyles;
    try {
      parsedStyles = parse(styleSource.source, { from: styleSource.path });
    } catch (error) {
      failures.push(`${relative(styleSource.path)} is not valid CSS: ${error.message}`);
      continue;
    }
...
function verifyRuntimeStyleTopology() {
  const srcRoot = path.join(root, "src");
  const cssFiles = listFiles(srcRoot)
    .map((file) => relative(file))
    .filter((file) => /\.(?:css|scss|less)$/i.test(file));
  assertSameSet(cssFiles, runtimeStyleFiles, "runtime stylesheet files");
  ...
  assertSameSet(
    [...new Set(cssImports.map(({ owner }) => owner))],
    [...runtimeStyleImportsByOwner.keys()],
    "runtime stylesheet import owners",
  );
  for (const [owner, expectedImports] of runtimeStyleImportsByOwner) {
  ...
  for (const cssImport of cssImports) {
    ...
    if (!expectedImports.includes(cssImport.resolved)) {
      failures.push(
        `${cssImport.owner} imports stylesheet ${cssImport.specifier}; expected an approved runtime stylesheet`,
      );
    }
  }
}
...
function verifyUnifiedSidebarContract() {
  ...
  if (/\.ops-shell\s+\.ops-sidebar/.test(styles)) {
    failures.push(
      "src/shared/ui/system.css has broad .ops-shell .ops-sidebar selectors; scope the unified side menu explicitly",
    );
  }
  if (
...
function verifySubmissionVisualBypassContract() {
  ...
  for (const checkedRoot of checkedRoots) {
    if (!fs.existsSync(checkedRoot)) {
      failures.push(`${relative(checkedRoot)} is missing`);
      continue;
    }
...
function readRequiredStyle(relativePath) {
  const absolutePath = path.join(root, relativePath);
  if (!fs.existsSync(absolutePath)) {
    failures.push(`${relativePath} is missing`);
    return "";
